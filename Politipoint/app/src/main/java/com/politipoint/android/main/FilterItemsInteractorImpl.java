...
public class FilterItemsInteractorImpl implements FilterItemsInteractor {

    public List<Member> members = new ArrayList<Member>();
    public int loop = 1;
    public  Map<String,String> states;

    @Override public void filterItems(final OnFinishedListener listener, final String val) {
        new Handler().postDelayed(new Runnable() {
            @Override public void run() {
                MemberService serviceAPI = RestClient.getClient();
                states = getStates();
                final String state = val;
